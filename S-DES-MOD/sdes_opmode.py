...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    for i in range(0, len(text), 8):
        block = sdes(text[i:i+8], key, MODE_ENCRYPT)
        result += block
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    for i in range(0, len(ciphertext), 8):
        block = sdes(ciphertext[i:i+8], key, MODE_DECRYPT)
        result += block
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(0, len(text), 8):
        if i == 0:
            temp_result = sdes(iv ^ text[i:i+8], key, MODE_ENCRYPT)
            result += temp_result
        else:
            block = sdes(temp_result ^ text[i:i+8], key, MODE_ENCRYPT)
            result += block
